...
{
    public class Fisher : Detector, IMetodoReconocimiento
    {
        ...
        public Fisher(string rutaCascadeClasifier) : base(rutaCascadeClasifier)
        { }
        public Fisher(string rutaCascadeClasifier, int NumeroDeComponentes) : base(rutaCascadeClasifier)
        {
            componentes = NumeroDeComponentes;
        }

        public Fisher(string rutaCascadeClasifier, string ArchivoDeEntrenamiento) : base(rutaCascadeClasifier)
        {
            archivoEntrenamiento = ArchivoDeEntrenamiento;
        }
        public Fisher(string rutaCascadeClasifier, string ArchivoDeEntrenamiento, int NumeroDeComponentes) : base(rutaCascadeClasifier)
        {
            archivoEntrenamiento = ArchivoDeEntrenamiento;
            componentes = NumeroDeComponentes;
        }
        ...
        public Bitmap Identificar(double Distancia, Bitmap IM, int ID_Buscar, ref bool Encontrado, ref double DistanciaObtenida)
        {
            if (archivoEntrenamiento != "")
            {
                ...
                {
                    faceRecognizer = new FisherFaceRecognizer(componentes, Distancia);
        ...
        private void ProcesoDeEntrenamiento(int ID_Persona, string rutaFotosEntrenamiento, int cantidadDeFotosUtilizar, string rutaResultados)
        {
            ...
            fotos = df.GetFiles("*.*", SearchOption.AllDirectories);
            int n = 0;
            Directory.CreateDirectory(rutaResultados + "\\ImagenesFISHER_" + ID_Persona.ToString()); // Se crea la carpeta de resultados
            ...
            foreach (FileInfo imagen in fotos)
            {
                ...
                if (rost != null)
                {
                    rost.Bitmap.Save(rutaResultados + "\\ImagenesFISHER_" + ID_Persona.ToString() + "\\IM_" + n.ToString() + ".jpg");
                    n++;
            ...
            // se define el nombre y ubicacion del archivo de entrenamiento (2)
            string archivoEntrenamiento = rutaResultados + "\\DatosEntrenamientoFISHER_" + ID_Persona.ToString() + ".trn";

            // Se entrena con las imagenes de los rostros (1) y se guarda el resultado en (2)
            GenerateTrainData(rutaResultados + "\\ImagenesFISHER_" + ID_Persona.ToString() + "\\", archivoEntrenamiento, ID_Persona);

            // Se obtiene la distancia de entrenamiento
            double distancia = ObtenerDistancia(rutaFotosEntrenamiento, archivoEntrenamiento, ID_Persona);

            // se crea el archivo con info adicional.
            StreamWriter wr = new StreamWriter(rutaResultados + "\\DatosEntrenaminentoFISHER.txt");
            ...
            wr.WriteLine("Distancia: " + distancia.ToString());
            wr.WriteLine("Metodo de reconocimineto: FISHER");
        ...
        {
            faceRecognizer = new FisherFaceRecognizer(componentes, double.PositiveInfinity);
        ...
        {
            faceRecognizer = new FisherFaceRecognizer(80, double.PositiveInfinity);
